...
class XAE_abstract(nn.Module):
    def __init__(self, cfg, log, device="cpu", verbose=1):
        ...
        if self.encoder_pretrain:
            ...
            self.pretrain_generator = torch.utils.data.DataLoader(
                self.train_data,
                self.encoder_pretrain_batch_size,
                num_workers=5,
                shuffle=True,
                pin_memory=True,
                drop_last=True,
    ...
    def train(self, resume=False):
        ...
        for epoch in range(start_epoch, self.num_epoch):
            ...
            for i, data in enumerate(self.train_generator):
                ...
                print(
                    "[%i/%i]\ttrain_main: %.4f\ttrain_penalty: %.4f"
                    % (
                        i + 1,
            ...
            if self.validate_batch:
                for i, data in enumerate(self.test_generator):
                    ...
                    if self.lamb > 0:
                        test_loss_penalty.append(
                            self.penalty_loss(
                                fake_latent, prior_z, self.test_generator.batch_size
                            ).item(),
                            n,
                        )
                    print(
                        "[%i/%i]\ttest_main: %.4f\ttest_penalty: %.4f"
                        % (i, len(self.test_generator), test_loss_main.avg, test_loss_penalty.avg),
                        end="\r",
                    )
                ...
                self.log.info(
                    "[%d/%d]\ttrain_main: %.6e\ttrain_penalty: %.6e\ttest_main: %.6e\ttest_penalty: %.6e"
                    % (
                        epoch + 1,
                        self.num_epoch,
                        train_loss_main.avg,
...
class CXAE_abstract(XAE_abstract):
    ...
    def encode(self, x, y):
        return torch.cat(
            (
                self.enc(torch.cat((self.embed_data(x), self.embed_condition(y)), dim=1)),
                self.embed_label(y),
            ),
            dim=1,
        )
