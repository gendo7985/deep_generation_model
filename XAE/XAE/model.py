...
from .util import inc_avg, save_sample_images, reparameterize, sigmoid
    ...
    def train(self, resume=False):
        ...
        for epoch in range(start_epoch, self.num_epoch):
            ...
            for i, data in enumerate(self.train_generator, 0):
                ...
                print(
                    "[%i/%i]\ttrain_mse: %.4f" % (i + 1, len(self.train_generator), train_loss.avg),
                    end="\r",
                )
            ...
            if self.validate_batch:
                ...
                self.log.info(
                    "[%d/%d]\ttrain_loss: %.6e\ttest_loss: %.6e"
                ...
                if len(self.tensorboard_dir) > 0:
                    ...
                    self.writer.add_images(
                        ...
                        (
                            np.concatenate(
                                (x.to("cpu").numpy()[0:16], sigmoid(recon.to("cpu").numpy())[0:16])
                            )
                        ),
                        epoch,
                    )
                ...
                if len(self.save_img_path) > 0:
                    ...
                    save_sample_images(
                        ...
                        epoch,
                        np.concatenate(
                            (x.to("cpu").numpy()[0:32], sigmoid(recon.to("cpu").numpy())[0:32])
                        ),
                    )
                    ...
                    test_dec = self.dec(prior_z).detach().to("cpu").numpy()
                    save_sample_images(
                        "%s/gen" % self.save_img_path, epoch, sigmoid(test_dec)[0:64]
                    )
                    plt.close()
    ...
    def __init__(self, cfg, log, device="cpu", verbose=1):
        ...
        if cfg["train_info"].getboolean("replace"):
            ...
            train_sampler = torch.utils.data.RandomSampler(
                self.train_data, replacement=True, num_samples=self.batch_size * it
            )
            ...
            if self.validate_batch:
                for i, (data, condition) in enumerate(self.test_generator):
                    with torch.no_grad():
                        ...
                        z = self.enc(
                            torch.cat((self.embed_data(x), self.embed_condition(y)), dim=1)
                        ).detach()
                        ...
                        recon = self.decode(
                            torch.cat((self.reparameterize(mu, logvar), y), dim=1)
                        ).detach()
                        logpx_z = torch.sum(self.recon_loss(recon, x), axis=[1, 2, 3])
                        D_KL = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), axis=1)
...
class SCWAE_GAN_abstract(WAE_GAN_abstract):
    ...
    def generate_prior(self, n):
        return torch.cat(
            (
                self.z_sampler(n, self.z_dim, device=self.device),
                self.y_sampler(n, self.y_dim, device=self.device),
            ),
            dim=1,
        )
    ...
    def pretrain_encoder(self):
        ...
        while True:
            for i, (data, condition) in enumerate(self.pretrain_generator):
                ...
                pz = torch.cat(
                    (
                        self.z_sampler(len(data), self.z_dim, device=self.device),
                        self.y_sampler(len(data), self.y_dim, device=self.device),
                    ),
                    dim=1,
                )
                x = data.to(self.device)
                ...
                qz_cov = torch.mean(
                    torch.matmul((qz - qz_mean).unsqueeze(2), (qz - qz_mean).unsqueeze(1)), dim=0
                )
                pz_cov = torch.mean(
                    torch.matmul((pz - pz_mean).unsqueeze(2), (pz - pz_mean).unsqueeze(1)), dim=0
                )
    ...
    def train(self, resume=False):
        ...
        for epoch in range(start_epoch, self.num_epoch):
            ...
            for i, (data, condition) in enumerate(self.train_generator):
                ...
                print(
                    "[%i/%i]\ttrain_mse: %.4f\ttrain_penalty: %.4f"
                    % (
                        i + 1,
                        len(self.train_generator),
                        train_loss_mse.avg,
                        train_loss_penalty.avg,
                    ),
                    end="\r",
                )
            ...
            if self.validate_batch:
                for i, (data, condition) in enumerate(self.test_generator):
                    ...
                    print(
                        "[%i/%i]\ttest_mse: %.4f\ttest_penalty: %.4f"
                        % (i, len(self.test_generator), test_loss_mse.avg, test_loss_penalty.avg),
                        end="\r",
                    )
                ...
                self.log.info(
                    "[%d/%d]\ttrain_mse: %.6e\ttrain_penalty: %.6e\ttest_mse: %.6e\ttest_penalty: %.6e"
                    % (
                        epoch + 1,
                        self.num_epoch,
                        train_loss_mse.avg,
                ...
                if len(self.save_img_path) > 0:
                    save_sample_images(
                        "%s/recon" % self.save_img_path,
                        epoch,
                        (np.concatenate((x.to("cpu").numpy()[0:32], recon.to("cpu").numpy()[0:32])))
                        * 0.5
                        + 0.5,
                    )
                    plt.close()
