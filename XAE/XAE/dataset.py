...
class MNIST(torch.utils.data.Dataset):
    def __init__(
        self, data_home, train=True, label=False, output_channels=1, portion=1.0, class_no=False
    ):
        self.label = label
        ...
        self.code = torch.from_numpy(np.concatenate([np.eye(10), np.zeros((1, 10))], axis=0)).type(
            torch.float32
        )
    ...
    def __getitem__(self, idx):
        if self.label:
            ...
            if self.output_channels > 1:
                return [
                    torch.from_numpy(2.0 * (self.data[idx, 1:785] / 255) - 1.0)
                    .reshape((1, 28, 28))
                    .type(torch.float32)
                    .repeat((self.output_channels, 1, 1)),
                    y,
                ]
            else:
                return [
...
class eYaleB(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.label:
            if self.output_channels > 1:
                return [
                    torch.from_numpy(2.0 * (self.data["image"][idx, :] / 255) - 1.0)
                    .reshape((1, 128, 128))
                    .type(torch.float32)
                    .repeat((self.output_channels, 1, 1)),
                    torch.cat(
                        (
                            self.code[self.data["person"][idx].astype(np.int)],
                            self.code2[self.data["pose"][idx].astype(np.int)],
                            torch.Tensor([self.data["azimuth"][idx] / 180.0]),
                            torch.Tensor([self.data["elevation"][idx] / 90.0]),
                        )
                    ).type(torch.float32),
                ]
            else:
                return [
                    torch.from_numpy(
                        (2.0 * (self.data["image"][idx, :] / 255) - 1.0).reshape((1, 128, 128))
                    ).type(torch.float32),
                    torch.cat(
                        (
                            self.code[self.data["person"][idx].astype(np.int)],
